...
/turf/open/floor/oworld/wood/setup_broken_states()
	return list("wood-broken", "wood-broken2", "wood-broken3", "wood-broken4", "wood-broken5", "wood-broken6", "wood-broken7")

/turf/open/floor/oworld/wood/examine(mob/user)
	. = ..()
	. += span_notice("There's a few <b>screws</b> and a <b>small crack</b> visible.")

/turf/open/floor/oworld/wood/screwdriver_act(mob/living/user, obj/item/I)
	if(..())
		return TRUE
	return pry_tile(I, user) ? TRUE : FALSE

/turf/open/floor/oworld/wood/try_replace_tile(obj/item/stack/tile/T, mob/user, params)
	if(T.turf_type == type)
		return
...
/turf/open/floor/oworld/wood/pry_tile(obj/item/C, mob/user, silent = FALSE)
	C.play_tool_sound(src, 80)
	return remove_tile(user, silent, (C.tool_behaviour == TOOL_SCREWDRIVER))

/turf/open/floor/oworld/wood/remove_tile(mob/user, silent = FALSE, make_tile = TRUE, force_plating)
	if(broken || burnt)
		broken = FALSE
		burnt = FALSE
		if(user && !silent)
...
/turf/open/floor/oworld/wood/cold
	temperature = 255.37

/turf/open/floor/oworld/wood/airless
	initial_gas_mix = AIRLESS_ATMOS

/turf/open/floor/oworld/wood/tile
	icon_state = "wood_tile"
	floor_tile = /obj/item/stack/tile/wood/tile

/turf/open/floor/oworld/wood/tile/setup_broken_states()
	return list("wood_tile-broken", "wood_tile-broken2", "wood_tile-broken3")

/turf/open/floor/oworld/wood/parquet
	icon_state = "wood_parquet"
	floor_tile = /obj/item/stack/tile/wood/parquet

/turf/open/floor/oworld/wood/parquet/setup_broken_states()
...
/turf/open/floor/oworld/wood/large
	icon_state = "wood_large"
	floor_tile = /obj/item/stack/tile/wood/large

/turf/open/floor/oworld/wood/large/setup_broken_states()
...
/turf/open/floor/oworld/grass/setup_broken_states()
	return list("sand")

/turf/open/floor/oworld/grass/Initialize(mapload)
	. = ..()
	spawniconchange()

/turf/open/floor/oworld/grass/proc/spawniconchange()
	icon_state = "grass[rand(0,3)]"

/turf/open/floor/oworld/grass/attackby(obj/item/C, mob/user, params)
...
/turf/open/floor/oworld/darkgrass/setup_broken_states()
	return list("sand")

/turf/open/floor/oworld/darkgrass/Initialize(mapload)
	. = ..()
	spawniconchange()

/turf/open/floor/oworld/darkgrass/proc/spawniconchange()
	icon_state = "grass[rand(0,3)]"

/turf/open/floor/oworld/darkgrass/attackby(obj/item/C, mob/user, params)
	if((C.tool_behaviour == TOOL_SHOVEL) && params)
		new ore_type(src, 2)
		playsound(src, 'sound/effects/shovel_dig.ogg', 50, TRUE)
		make_plating()
...
/turf/open/floor/oworld/indestructible/mud
	...
	icon_state = "mud"
//	------------------ LAVA FLOOR ------------------
/turf/open/floor/oworld/floor/white
	icon = 'modular_septic/icons/turf/oworld/flooring/tiles.dmi'
	icon_state = "white"
//	------------------ SNOW FLOOR ------------------
/turf/open/floor/oworld/snow
	gender = PLURAL
	name = "snow"
...
/turf/open/floor/oworld/snow/proc/spawniconchange()
	icon_state = "snow[rand(1,9)]"

/turf/open/floor/oworld/snow/try_replace_tile(obj/item/stack/tile/T, mob/user, params)
	return

/turf/open/floor/oworld/snow/crowbar_act(mob/living/user, obj/item/I)
	return
...
/turf/open/floor/oworld/desert/setup_broken_states()
	return list("sand")

/turf/open/floor/oworld/desert/Initialize(mapload)
	. = ..()
	spawniconchange()

/turf/open/floor/oworld/desert/proc/spawniconchange()
